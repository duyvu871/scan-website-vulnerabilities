...
const ALLOWED_ORIGINS_AND_PERMISSIONS = new Map<
    ...
    >
>(
    isDev ? [["localhost", new Set(
        ["clipboard-read", "media", "geolocation", "midi", "midiSysex", "pointerLock", "fullscreen", "openExternal"]
    )]] : [[
            "localhost",
            new Set(
                ["clipboard-read", "media", "geolocation", "midi", "midiSysex", "pointerLock", "fullscreen", "openExternal"]
...
pem.config({
    pathOpenSSL: packagePath.openssl
})
    ...
    contents.session.setPermissionRequestHandler(
        (webContents, permission, callback) => {
            ...
            const permissionGranted =
                !!ALLOWED_ORIGINS_AND_PERMISSIONS.get(origin)?.has(permission as any);
